...
A custom node + ready-to-use workflows for [ComfyUI](https://github.com/comfyanonymous/ComfyUI) that lets you describe several image variations in one text box and generate all of them in a single run — instead of manually editing and re-queuing the prompt for each one.
...
![Variant Prompt Builder node](docs/screenshots/node-ui.png)

### Example output (same seed)

![Example output with same seed](docs/screenshots/example-output-same-seed.png)

![Another example with same seed](docs/screenshots/example-output-same-seed_2.png)
...
## Three ready-to-use workflows (pick the one that matches your models)

The node itself is model-agnostic. To make life easy we ship **three complete, user-friendly workflows**:

| Workflow file | Best for | How models are loaded |
|---------------|----------|-----------------------|
| `workflows/flux.json` | Flux / Flux.2 family | Specialized Flux nodes (UNET + CLIP + VAE loaders + Flux scheduler & latent) |
| `workflows/aio.json` | All-in-one / single-file checkpoints (most SD1.5, SDXL, Pony, Illustrious, many community AIO packs) | **One node** – CheckpointLoaderSimple |
| `workflows/checkpoint.json` | Models that need separate files | **Three nodes** – UNET/Model loader + CLIP loader + VAE loader |

All three are designed the same way:

1. Install the custom node (one folder copy).
2. Open the matching workflow JSON in ComfyUI.
3. Select your model(s) in the loader node(s).
4. Type your general text + numbered variants.
5. Queue → get every variant generated automatically.

No extra wiring, no loop nodes, no code. Download → install → generate.

![AIO workflow example](docs/screenshots/workflow-aio.png)
...
- **Three model-type workflows** — Flux, AIO (single loader), and separate-checkpoint versions so you can start generating immediately.
...
2. That prompt list is wired into the workflow's existing `CLIPTextEncode` node, and the seed list into the noise / sampler seed input.
...
```
Variant-Prompt-Builder-ComfyUI/
├── custom_nodes/
...
├── workflows/
│   ├── flux.json                         # Flux / Flux.2 ready workflow
│   ├── aio.json                          # All-in-one / single-file checkpoint workflow
│   ├── checkpoint.json                   # Separate model + CLIP + VAE loaders
│   └── README.md
├── examples/
...
│       ├── node-ui.png
│       ├── workflow-aio.png
│       ├── example-output-same-seed.png
│       └── example-output-same-seed_2.png
...
Open one of the three ready workflows in ComfyUI:

- `workflows/flux.json`          → if you use Flux / Flux.2 models
- `workflows/aio.json`           → if you use all-in-one / single-file checkpoints
- `workflows/checkpoint.json`    → if you load model, CLIP and VAE as three separate files

Then simply select your model(s) in the loader node(s) and start generating.
...
- **Generative AI / diffusion pipelines** — hands-on work with Flux, node-based visual workflow engineering in ComfyUI, support for multiple model loading styles.
